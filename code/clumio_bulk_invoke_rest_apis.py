...
def get_endpoint_mappings(client: clumioapi_client.ClumioAPIClient) -> dict:
    """Return a mapping of endpoint labels to client list methods (lazy, not invoked)."""
    return {
        ...
        'list_protection_group_s3_assets': (
            client.protection_groups_s3_assets_v1.list_protection_group_s3_assets
        ),
        'list_backup_aws_dynamodb_tables': (
            client.backup_aws_dynamodb_tables_v1.list_backup_aws_dynamodb_tables
        ),
        'list_backup_protection_groups': (
            client.backup_protection_groups_v1.list_backup_protection_groups
        ),
        'list_backup_aws_ebs_volumes': (
            client.backup_aws_ebs_volumes_v2.list_backup_aws_ebs_volumes
        ),
        'list_backup_aws_ec2_instances': (
            client.backup_aws_ec2_instances_v1.list_backup_aws_ec2_instances
        ),
        'list_backup_aws_rds_resources': (
            client.backup_aws_rds_resources_v1.list_backup_aws_rds_resources
        ),
    }
...
def lambda_handler(events: EventsTypeDef, context: LambdaContext) -> dict[str, Any]:
    ...
    # Get the REST API mappings (lazy callables, not pre-invoked).
    mappings = get_endpoint_mappings(client)
    ...
    # Invoke only the selected endpoint.
    try:
        logger.info('[%s] Sending request...', endpoint)
        response = mappings[endpoint](filter=common.make_filter(filters), limit=limit)
    except clumio_exception.ClumioException as e:
        logger.info('[%s] Request failed with exception: %s', endpoint, e)
        return {'status': 500, 'msg': f'API error - {e}'}
    logger.info('[%s] Request successful.', endpoint)
    return {'status': 200, 'response': response.dict() if hasattr(response, 'dict') else None}
